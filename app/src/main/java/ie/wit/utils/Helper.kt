...
fun uploadImageView(app: CarBookingApp, imageView: ImageView) {
    ...
    uploadTask.addOnFailureListener { object : OnFailureListener {
        override fun onFailure(error: Exception) {
            Log.v("Booking", "uploadTask.exception" + error)
        }
    }
...
fun writeImageRef(app: CarBookingApp, imageRef: String) {
...
fun updateAllBookings(app: CarBookingApp) {
    val userId = app.currentUser!!.uid
    ...
    bookingRef.equalTo(userEmail).addListenerForSingleValueEvent(
    ...
    userBookingRef.addListenerForSingleValueEvent(
        object : ValueEventListener {
            override fun onCancelled(error: DatabaseError) {}
            override fun onDataChange(snapshot: DataSnapshot) {
                snapshot.children.forEach {
...
fun validatePhoto(app: CarBookingApp, activity: Activity) {
...
fun checkExistingPhoto(app: CarBookingApp, activity: Activity) {
